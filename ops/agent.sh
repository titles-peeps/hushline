...
# Detect repo from git remote; fallback exactly as requested
REPO="${REPO:-$(git remote get-url origin 2>/dev/null | sed -n 's#.*github.com[:/]\(.*\.git\)#\1#p' | sed 's/\.git$//')}"
REPO="${REPO:-titles-peeps/hushline}"

MODEL="${AIDER_MODEL:-ollama_chat/qwen2.5-coder:7b-instruct}"  # native Ollama adapter
...
# Try to hint files mentioned in the issue body (generic; no CSS special-casing)
...
# Single pass with Aider (avoid heavy resource use)
...
# If nothing changed, comment & exit cleanly
